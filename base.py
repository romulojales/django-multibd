...
from django.db.models.base import method_get_order, method_set_order, \
    get_absolute_url, model_unpickle
...
class MultiBdModelBase(base.ModelBase):
     ...
     def _prepare(cls):
        ...
        signals.class_multibd_prepared.send(sender=cls)
        
class MultiBdModel(object):

    __metaclass__ = MultiBdModelBase
    _deferred = False
    ...
    def save_base(self, raw=False, cls=None, origin=None,
        ...
        if origin:
            signals.post_save.send(sender=origin, instance=self,created=(not record_exists), raw=raw)
